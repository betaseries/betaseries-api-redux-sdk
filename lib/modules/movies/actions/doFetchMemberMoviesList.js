import constants from '../constants';
import ApiFetch from '../../../utils/fetch/ApiFetch';

const defaultProps = {
  state: 0,
  start: 0,
  limit: 50,
  order: 'alphabetical'
};

/**
 * Retrieve movies list of the member
 *
 * **Dispatch**: `FETCH_MEMBER_MOVIES_LIST`
 *
 * @alias module:Movies.doFetchMemberMoviesList
 * @category actions
 *
 * @example
 * BetaSeries.getAction('movies', 'doFetchMemberMoviesList')({ memberId: 1 });
 *
 * @param {Object}  [obj]             Accept the following:
 * @param {Number}  [obj.memberId]    Member ID
 * @param {Number}  [obj.state]       State of movie
 *   (`0` = to see, `1` = seen, `2` = doesn't want to see (`0` by default))
 * @param {Number}  [obj.start]       Used for paginate (`0` by default)
 * @param {Number}  [obj.limit]       Used for paginate (optional)
 * @param {String}  [obj.order]       Sort order (`alphabetical`, `popularity`, `added` (optional))
 *
 * @returns {Promise}
 */
const doFetchMemberMoviesList = ({ memberId, ...props }) =>
  dispatch =>
    ApiFetch.get('movies/member', {
      id: memberId,
      ...defaultProps,
      ...props
    }).then(response =>
      dispatch({
        type: constants.FETCH_MEMBER_MOVIES_LIST,
        payload: {
          ...props,
          memberId,
          movies: response.movies,
          total: response.total,
          totalMissingMovies: response.totalMissingMovies
        }
      }));

export default doFetchMemberMoviesList;
