...
export default function collectionsCollectionsReducer(
  ...
  switch (action.type) {
    ...
    case constants.FETCH_SUBSCRIBED_COLLECTIONS_LIST: {
      const stateCollections = Object.values(state);

      const newCollections = stateCollections.map(collection => {
        const payloadC = _find(action.payload.collections, {
          id: collection.id
        }) || {};
