...
const doMarkEpisodeAsWatched = ({ episodeId, bulk = true, ...props }) =>
  (dispatch, getState) => {
    ...
    return ApiFetch.post('episodes/watched', {
      ...
      if (error && error.statusCode === 400 && error.message && error.message.includes('déjà marqué')) {
        // console.debug("[BetaSeries SDK] Ignoré : L'utilisateur a déjà marqué cet épisode comme vu.");
