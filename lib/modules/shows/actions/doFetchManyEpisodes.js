 ...
 * @returns {Promise}
 */
const doFetchManyEpisodes = ({ episodeIds }) =>
  (dispatch, getState) => {
    const state = getState();

    // fetch episodes only if does not exist on reducers
    const cleanEpisodeIds = _difference(
      episodeIds,
      Object.keys(state.showsEpisodes).map(episode => parseInt(episode, 10))
    );
    ...
    return ApiFetch.get('episodes/display', {
      ...
      dispatch({
        ...
        payload: {
          ...
          episodes: cleanEpisodeIds.length === 1
            ? [response.episode]
            : response.episodes
        }
      }));
