...
const doFetchManyEpisodes = ({ episodeIds }) => (dispatch, getState) => {
  ...
  const cleanEpisodeIds = !Object.prototype.hasOwnProperty.call(state, 'episodes') ? episodeIds : (
    _difference(episodeIds, Object.keys(state.episodes).map(episode => parseInt(episode, 10)))
  );
