...
const actionFile = '../lib/modules/movies/actions/doFetchMemberMoviesList';
const moviesFixture = require('../../../fixtures/movies.json');

describe('Retrieve movies list of the member', () => {
  /**
   * getInstance method
   */
  ...
  describe('call api with member ID', () => {
    ...
    it('validate action', () => {
      expect(action.type).to.equal('FETCH_MEMBER_MOVIES_LIST');
    ...
    it('validate members reducer with existing values', () => {
      const stateMembersReducer = membersReducer({ 1: [7094] }, action);
      expect(stateMembersReducer).to.deep.equal({ 1: [7094, 4656] });
    });
